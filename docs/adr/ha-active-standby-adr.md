...
**Exception:** hub-issued SA **token values** are not mirrored. A token signed by the Active is invalid on the Standby, so the mirror carries the `ServiceAccount` + RBAC (the identity and authorization) and each hub mints its own token (Decision 7).

The token `Secret` itself *is* mirrored, as an empty shell: name, namespace, `type`, and the `kubernetes.io/service-account.name` annotation cross; `.data` is dropped, and so is the `kubernetes.io/service-account.uid` annotation, which the token controller validates against the *local* account's UID and which a mirrored `ServiceAccount` — created fresh on the Standby — never matches. The Standby's own token controller then populates the shell with a token valid on the Standby. This is what lets a Standby hold a working worker credential *before* any failover, without unfencing its registration reconciler; it is the mechanism Open Issue 4's credential prerequisite was missing. Carrying the shell rather than synthesising one keeps the mirror's own model intact — the object exists on the Active, so the prune diff and the delete path need no special case.

Two properties this depends on, both non-obvious enough to state:

- **The shell is create-only.** The mirror's update path is an unconditional full write, and the informers resync periodically, so re-applying the Active's version would clear the minted token on a timer; the token controller would mint a fresh one and silently invalidate whatever copy a worker was already using. The Standby's copy is seeded once and owned locally thereafter.
- **The Standby caches every `Secret` cluster-wide.** No single server-side selector admits both the SA-token shells and the unlabeled certificate `Secret`s the cert-generator job creates. This widens what the Standby *caches*, not what it may *read*: the least-privilege `ClusterRole` implementing Decision 2's remote identity already grants cluster-wide `Secret` read, because RBAC can scope Secrets neither by `.type` nor by namespace label, and it names that as a tradeoff rather than burying it. Active-minted token values are additionally stripped on the way into the cache, so they are never held in the Standby's process at all.

**What "each hub mints its own token" requires of the registration path.** This is a requirement on the implementation, not a property it already has. The registration path must make the token `Secret`'s creation conditional on **that Secret's** existence, not on the `ServiceAccount`'s: a routine that only mints one inside its `ServiceAccount`-not-found branch will find a mirrored account already present, skip the branch, and never mint a token there at all. A promoted hub then fails every reconcile of every registered cluster, permanently, while reporting a successful promotion. Shell mirroring means the Secret is normally already there and the branch simply finds it, but the correction is still required: it covers a hub promoted from a mirror that predates shell mirroring, and it independently repairs any standalone hub whose token Secret was removed by hand, which otherwise has no recovery path.
...
4. **Worker's failover-time detection of `status.activeController` — direction chosen, two details remain.** The field lives on the hub-side `Cluster` CR; a worker watching only the currently-Active hub can't observe a status change on an API server that just died, since the update lands on the *other* hub's copy instead. Direction: the worker runs two permanent, lightweight watches — one against each of its two fixed, pre-provisioned hub endpoints, never against "the Active" or "the Standby" since that role can flip. Each hub writes `activeController` about **itself**, locally, only when it currently holds leadership; a Standby's own copy (populated by `StateMirror` from the Active) still shows the Active's identity, not its own, so the rule "trust whichever endpoint is reachable and reports `activeController == that endpoint's own identity`" resolves correctly without the worker ever needing to know which role either hub currently holds — and without any failure-timeout tuning, since it's reacting to a self-declaration, not inferring a death. Two things this still depends on:
   ...
   - **Credential prerequisite — hub side settled, delivery still open.** This assumes the worker already holds a valid, durable credential against both hubs (Decision 7). The question of *how* the Standby comes to hold one while its own registration reconciler is `IsLeader()`-gated off is answered in Decision 6: it does not mint one from a fenced reconciler, it mirrors the token `Secret`'s shell and lets its own token controller populate it. A Standby therefore holds a worker credential valid on itself continuously, before any failover.
     What remains open is **delivery**: getting that Standby-valid token onto the worker at registration time, as a second mounted credential. That is not code in either the controller or the worker-operator — it is the documented registration procedure and the Helm charts, which neither repo owns. It has to be settled with whoever ships those. Until it is, the chain can be completed by hand for demonstration purposes, which is enough to prove the two ends but not to ship. Consumption of the second credential — mounting it, and rebuilding the connection against the promoted hub — is `worker-operator` #467.
