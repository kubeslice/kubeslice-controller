...
var _ = Describe("E2E: Cluster CRD lifecycle", Ordered, func() {
	...
	It("should reconcile and update Cluster status", func() {
		By("Waiting for Cluster status to show registration in progress or registered")
		Eventually(func() (controllerV1.RegistrationStatus, error) {
			f := &controllerV1.Cluster{}
			if err := k8sClient.Get(ctx, types.NamespacedName{Name: name, Namespace: ns}, f); err != nil {
				return "", err
			}
			return f.Status.RegistrationStatus, nil
		}, 120*time.Second, 5*time.Second).Should(Or(
			Equal(controllerV1.RegistrationStatusInProgress),
			Equal(controllerV1.RegistrationStatusRegistered),
		))
