...
type IWorkerSliceGatewayService interface {
	...
	CreateMinimumWorkerSliceGateways(ctx context.Context, sliceName string, clusterNames []string, namespace string,
		label map[string]string, clusterMap map[string]int, subnetMap map[string]string, sliceSubnet string, clusterCidr string, sliceGwSvcTypeMap map[string]*controllerv1alpha1.SliceGatewayServiceType) (ctrl.Result, error)
	...
	BuildNetworkAddresses(sliceSubnet, sourceClusterName, destinationClusterName string,
		clusterMap map[string]int, subnetMap map[string]string, clusterCidr string) util.WorkerSliceGatewayNetworkAddresses
}
...
func (s *WorkerSliceGatewayService) CreateMinimumWorkerSliceGateways(ctx context.Context, sliceName string,
	...
	subnetMap map[string]string, sliceSubnet string, clusterCidr string, sliceGwSvcTypeMap map[string]*controllerv1alpha1.SliceGatewayServiceType) (ctrl.Result, error) {
	...
	_, err = s.createMinimumGatewaysIfNotExists(ctx, sliceName, clusterNames, namespace, label, clusterMap, subnetMap, sliceSubnet, clusterCidr, sliceGwSvcTypeMap)
	if err != nil {
		return ctrl.Result{}, err
	}
	return ctrl.Result{}, nil
...
func (s *WorkerSliceGatewayService) createMinimumGatewaysIfNotExists(ctx context.Context, sliceName string,
	...
	for i := 0; i < noClusters; i++ {
		for j := i + 1; j < noClusters; j++ {
			...
			gatewayAddresses := s.BuildNetworkAddresses(sliceSubnet, sourceCluster.Name, destinationCluster.Name, clusterMap, subnetMap, clusterCidr)
...
func (s *WorkerSliceGatewayService) BuildNetworkAddresses(sliceSubnet, sourceClusterName, destinationClusterName string,
	...
	// Use subnetMap if available (preferred for Dynamic IPAM and consistent Static IPAM)
	if subnetMap != nil {
		serverSubnet = subnetMap[sourceClusterName]
		clientSubnet = subnetMap[destinationClusterName]
	} else {
		// Fallback to calculation (Legacy Static IPAM)
