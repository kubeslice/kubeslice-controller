...
func testCreateMinimumWorkerSliceGatewaysAlreadyExists(t *testing.T) {
	...
	result, err := workerSliceGatewayService.CreateMinimumWorkerSliceGateways(ctx, "red", clusterNames, requestObj.Namespace, label, clusterMap, "10.10.10.10/16", "/16", nil, []util.GatewayPair{{Source: "cluster-1", Target: "cluster-2"}})
	expectedResult := ctrl.Result{}
	require.NoError(t, nil)
	require.Equal(t, result, expectedResult)
	require.Nil(t, err)
